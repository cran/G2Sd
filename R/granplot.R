granplot <-
function(x,xc=1,hist=TRUE,cum=TRUE,main="",col.cum="red",col.hist="gray",cexname=0.9) 
  {
...
if (hist == TRUE & cum == TRUE) {
  ...
  barplot(x[, xc] * ((100/max(x[, xc]))), xlab = "Particule size (microns)", 
          ...
          main = main, col = col.hist,cex.names=cexname,font.lab=2,cex.lab=1.3)
  ...
  mtext("(% cum) ", 4, line = 2, col = col.cum,font=2,cex=1.3)
...
if (hist == FALSE & cum == TRUE) {
  plot(class.weight.cum, type = "l", lwd = 1, xlab = "Particule size (microns)", 
       ...
       xlim = c(0,length(x[, xc]) ), main = main, col = col.cum,cex.axis=cexname,font.lab=2,cex.lab=1.3)
...
if (hist == TRUE & cum == FALSE) {
  barplot(round(x[, xc],2), xlab = "Particule size (microns)", ylab = "Weight (g)", 
          names.arg = um, las = 2, main = main, col = col.hist,cex.names=cexname,font.lab=2,cex.lab=1.3)
